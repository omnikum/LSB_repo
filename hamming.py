...
CHUNK_LENGTH = 4  # длина блока кодирования

assert not CHUNK_LENGTH % 4, 'Длина блока должна быть кратна 4'  # проверка кратности длины блока кодирования
...
def decode_hamming(encoded, fix_errors=True):
    ...
    format_chunks = ''.join(clean_chunk_list)

    for clean_char in [format_chunks[i:i + 8] for i in range(0, len(format_chunks), 8)]:
...
def encode_hamming(message):
    ...
    assert not len(message) * 4 % CHUNK_LENGTH, 'Длина кодируемых данных должна быть кратна длине блока кодирования'
    text_bin = ''.join([bin(ord(c))[2:].zfill(8) for c in message])
    result = ''
    print("Binary message is:", text_bin)
    for chunk_bin in chunk_iterator(text_bin):
        chunk_bin = set_check_bits(chunk_bin)
        result += chunk_bin
    print("Before Error: ", result)
    result = set_errors(result)
    print("Encoded message is: ", result)
    print("Hamming code [{0},{1}]".format(pow(2, len(CHECK_BITS))-1, pow(2, len(CHECK_BITS)) - 1 - len(CHECK_BITS)))
    return result